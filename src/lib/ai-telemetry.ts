...
/**
 * Clear all telemetry for a project.
 * Useful for testing or data reset.
 *
 * @param projectId - Project ID to clear
 */
export async function clearTelemetry(projectId: number): Promise<void> {
  const projectPath = getCurrentProjectPath();
  if (!projectPath) return;

  const db = await getDatabase(projectPath);
  await db.execute(
    'DELETE FROM ai_telemetry WHERE project_id = $1',
    [projectId]
  );
}

/**
 * Get recent telemetry entries for debugging.
 *
 * @param projectId - Project ID to query
 * @param limit - Maximum entries to return (default: 10)
 * @returns Recent telemetry entries
 */
export async function getRecentTelemetry(
  projectId: number,
  limit: number = 10
): Promise<Array<TelemetryEntry & { id: number; createdAt: string }>> {
  const projectPath = getCurrentProjectPath();
  if (!projectPath) return [];
  ...
  return rows.map(row => ({
    ...
    operation: row.operation as TelemetryOperation,
    provider: row.provider,
    model: row.model,
    success: row.success === 1,
    errorType: row.error_type as TelemetryErrorType | undefined,
