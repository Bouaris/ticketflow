...
must_haves:
  truths:
    ...
    - "All usage sites in this plan (useAIQuestioning, OnboardingWizard, ProjectWorkspace) import from constants instead of hardcoding string literals (AISettingsModal handled by plan 33-01)"
...
<task type="auto">
  ...
  <files>src/constants/storage.ts, src/hooks/useAIQuestioning.ts, src/components/onboarding/OnboardingWizard.tsx, src/components/workspace/ProjectWorkspace.tsx</files>
  <action>
**storage.ts:**
...
Note: AISettingsModal.tsx storage key replacement (lines 33, 57) has been moved to Plan 33-01 which owns that file exclusively, avoiding parallel execution conflicts.
  </action>
  ...
  <done>STORAGE_KEYS has QUESTIONING_MODE and LOCALE; useAIQuestioning, OnboardingWizard, and ProjectWorkspace import from constants; local QUESTIONING_STORAGE_KEY removed from useAIQuestioning.ts; AISettingsModal storage keys handled by plan 33-01; `pnpm build` passes.</done>
</task>
