...
must_haves:
  truths:
    ...
    - "SECURITY.md documents tracked Cargo vulnerabilities (bytes, time, rkyv, rsa) in dependency exceptions"
  artifacts:
    - path: ".github/workflows/ci.yml"
      provides: "SHA-pinned GitHub Actions"
      contains: "actions/checkout@"
    - path: "SECURITY.md"
      provides: "Accepted risk documentation for SEC-D2 and SEC-D10, plus Cargo vulnerability tracking"
...
<task type="auto">
  <name>Task 2: Document accepted security risks and tracked Cargo vulnerabilities in SECURITY.md</name>
...
**Additionally**, update the "Dependency Security" > "Current exceptions" section in SECURITY.md to document tracked Cargo crate vulnerabilities:
- **bytes**: Note whether the fix was pulled in by cargo update (>=1.11.1), or if it's still blocked by upstream Tauri.
- **time**: Same — note resolution status (>=0.3.41).
...
NOTE: The actual resolution status of bytes and time depends on what `cargo update` achieves in plan 36-02. Write the initial tracking entries here; plan 36-02 executor will update the status fields after running cargo update/audit if needed.
...
3. "Dependency Security" / "Current exceptions" section covers bytes, time, rkyv, rsa tracking status
4. Security Audits section references Phase 31
5. Footer shows version 2.2.1, date 2026-02-19
  </verify>
  <done>SECURITY.md documents both SEC-D2 (IPC api_key) and SEC-D10 (devtools) as accepted risks with full rationale. Cargo vulnerability tracking (bytes, time, rkyv, rsa) documented in dependency exceptions. Security audit history updated.</done>
</task>
...
5. SECURITY.md "Current exceptions" section covers bytes, time, rkyv, rsa
6. `pnpm build` passes (no source changes, just config and docs)
</verification>
...
- SECURITY.md tracks Cargo vulnerabilities (bytes, time, rkyv, rsa) in dependency exceptions
