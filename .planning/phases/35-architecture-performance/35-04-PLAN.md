...
**Part B: Integrate @tanstack/react-virtual**

Replace the current `<tbody>` rendering with a virtualized approach:
  ...
  return (
    <div ref={parentRef} className="flex-1 overflow-auto">
      <table className="min-w-full">
        ...
        <tbody
          className="bg-surface divide-y divide-outline"
...
Add `style` to `ListViewRowProps` interface: `style?: React.CSSProperties;` and apply it to the `<tr>` element.
...
- `position: 'absolute'` on rows + `translateY` is the standard @tanstack/react-virtual pattern for table rows.
- The `<tbody>` needs explicit height from `virtualizer.getTotalSize()` and `position: relative` to serve as the positioning context.
- Remove the `AnimatePresence` + `motion.tr` wrappers — they conflict with virtualized positioning. The row enter/exit animations are sacrificed for virtualization performance.
