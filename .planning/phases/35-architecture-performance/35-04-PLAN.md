...
- Remove the `SPRING_PRESETS` import — it is only used in `motion.tr` which is being removed by this task. After removing `motion.tr`, `SPRING_PRESETS` has no remaining usage in ListView.tsx.
  </action>
  ...
  <done>ListView uses @tanstack/react-virtual with ~52px row estimate and 10 overscan. ListViewRow is memo-wrapped. AnimatePresence removed from row level. No unused imports remain in ListView.tsx (including SPRING_PRESETS, motion, AnimatePresence). Build passes.</done>
</task>
