...
must_haves:
  truths:
    ...
    - "dark_mode_toggled event fires when user changes theme in AppSettingsModal with theme value as metadata"
    - "project_created event fires in App.tsx handleProjectSelect when a new project is created"
  artifacts:
...
**dark_mode_toggled event (TELE-06):**
Since AppSettingsModal.tsx is owned by this plan, add `dark_mode_toggled` instrumentation here. Find the theme selector/toggle where `setTheme()` is called (the theme option click handler). Wrap it to also fire a telemetry event:
```typescript
// In the theme option click handler, after calling setTheme(opt.value):
track('dark_mode_toggled', { theme: opt.value });
```
This ensures the event fires with the selected theme value (`'light'`, `'dark'`, or `'system'`) whenever the user changes the theme.

**project_created event (TELE-05):**
Since App.tsx is owned by this plan, add `project_created` instrumentation in `handleProjectSelect`. The actual creation point is when `types && types.length > 0` (indicating a new project with type config, not opening an existing project). Add inside the `if (types && types.length > 0)` branch:
```typescript
// After typeConfig.initializeWithTypes(projectPath, types):
track('project_created');
```
Import `track` alongside the other telemetry imports already added in Task 1 (App.tsx).
  </action>
  <verify>
`pnpm build` passes. AppSettingsModal shows a Privacy section with a toggle switch. Toggling off writes 'declined' to localStorage and shows confirmation message. Toggling on writes 'granted' and re-initializes telemetry. `grep "dark_mode_toggled" src/components/settings/AppSettingsModal.tsx` returns at least 1 match. `grep "project_created" src/App.tsx` returns at least 1 match.
  </verify>
  <done>
AppSettingsModal.tsx has a Privacy section with an on/off toggle. Disabling shows "Telemetry disabled. No data will be sent." confirmation. Enabling re-initializes telemetry. State syncs with localStorage on modal open. The `dark_mode_toggled` event fires when the user changes theme, with `{ theme }` metadata. The `project_created` event fires in App.tsx `handleProjectSelect` when a new project is created (types present).
  </done>
</task>
