...
<objective>
Create AppSettingsModal and AISettingsModal with all sub-components as standalone new files. These modals are NOT yet wired into App.tsx — that happens in Plan 02. This plan focuses on building the complete UI components.
...
- Use `Modal` with `size="lg"` and title from i18n (use t.settings.title for now, will be updated in Plan 02)
...
**IMPORTANT:**
- Use `useTranslation()` for ALL user-facing strings. For NEW strings that don't exist yet in i18n types, use hardcoded English strings for now (they'll be added to i18n in Plan 02). Comment with `// TODO: i18n` next to hardcoded strings.
...
- All components must support dark mode via the existing theme tokens.
  ...
  <verify>
Run `pnpm build` — must pass. New files should compile even though they're not imported anywhere yet (tree-shaking will exclude them, but TypeScript still type-checks).

Actually, since they are new files not imported by anything, TypeScript may not check them. To verify: add a temporary import in a test file or check manually that the files have no red squiggles. At minimum: `npx tsc --noEmit` should pass.
  </verify>
  <done>
...
2. All five files exist: AppSettingsModal.tsx, AISettingsModal.tsx, ProviderCard.tsx, CustomProviderForm.tsx, CustomProviderList.tsx
3. No modifications to existing files
4. Each component uses Modal, Icons, and ai-provider-registry imports correctly
</verification>
