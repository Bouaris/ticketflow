...
<task type="auto">
  ...
  **Step 4: Fix AIContextIndicator test 76**
  ...
  Test 76 (`renders context badge after loading`) awaits `findByText('CLAUDE')` which times out because `loadProjectContext` and `getContextStatus` from `../../lib/ai-context` are not mocked. The async call runs in the component but resolves with real module behavior which returns `{files: []}` in non-Tauri mode (returning null/empty).

  Add a `vi.mock` for `../../lib/ai-context` at the top of the AIContextIndicator test section (or at file top with existing mocks), returning:
  ...
  Since `vi.mock` is hoisted in test files, it will be available for all AIContextIndicator tests.

  **IMPORTANT:** The `ai-context` mock MUST be placed with the other top-level `vi.mock` calls in `components.test.tsx`, NOT inside a `describe` block.
  </action>
  <verify>
