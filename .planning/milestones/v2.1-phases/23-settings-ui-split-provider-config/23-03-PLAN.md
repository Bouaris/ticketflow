...
must_haves:
  truths:
    ...
    - "No imports of deprecated modules (useProjectAIConfig, projectAIConfig) remain in codebase"
  artifacts:
    - path: "src/App.tsx"
  ...
  key_links:
    ...
    - from: "src/components/workspace/ProjectWorkspace.tsx"
      to: "src/lib/ai.ts"
      via: "hasApiKey(getProvider()) for badge state"
      pattern: "hasApiKey.*getProvider"
---
...
Output: Fully functional settings split with App Settings (Cmd+,) and AI Settings (header button) accessible separately. All deprecated code removed.
</objective>
...
# Plan 01 and 02 summaries (component files + i18n)
@.planning/phases/23-settings-ui-split-provider-config/23-01-SUMMARY.md
@.planning/phases/23-settings-ui-split-provider-config/23-02-SUMMARY.md
...
4. **Verify** the `getProvider` import resolves correctly — it should come from `../../lib/ai` (exported as a named function). If not found, check the 23-01-SUMMARY for the correct import path.
5. In the workspaceActions object (where `openSettings: onOpenSettings`), keep `openSettings: onOpenSettings` — the command palette "Settings" command still opens App Settings via `actions.openSettings`.
  </action>
  <verify>
...
- Verify `getProvider` import in ProjectWorkspace.tsx compiles (no unresolved import)
  </verify>
  <done>
App.tsx renders AppSettingsModal and AISettingsModal instead of SettingsModal. Header has AI Settings button with amber badge. ProjectWorkspace passes onOpenAISettings callback and computes badge state via hasApiKey(getProvider()). Command palette "Settings" opens App Settings.
  </done>
</task>

<task type="auto">
  <name>Task 2: Remove project-level AI selector, delete deprecated files, clean exports</name>
  <files>
    src/components/settings/ProjectSettingsModal.tsx
    src/components/settings/SettingsModal.tsx
    src/hooks/useProjectAIConfig.ts
...
- **Delete** `src/components/settings/SettingsModal.tsx` — fully replaced by AppSettingsModal + AISettingsModal.

**Delete deprecated files and clean barrel exports:**

1. **Delete** `src/hooks/useProjectAIConfig.ts` — no longer needed
...
Verify:
- No imports of `useProjectAIConfig`, `projectAIConfig.ts`, `AVAILABLE_MODELS`, `DEFAULT_MODELS` remain in codebase: `grep -rn "useProjectAIConfig\|projectAIConfig\|AVAILABLE_MODELS\|DEFAULT_MODELS" src/`
- ProjectSettingsModal no longer shows AI provider selector
- `src/components/settings/SettingsModal.tsx` is deleted
- `src/hooks/useProjectAIConfig.ts` is deleted
- `src/types/projectAIConfig.ts` is deleted
  </verify>
  <done>
ProjectSettingsModal stripped of AI provider selector (only shows project name, context files, GSD, type config link). Deprecated files deleted (SettingsModal.tsx, useProjectAIConfig.ts, projectAIConfig.ts). Barrel exports cleaned. ai.ts deprecated stubs removed. No deprecated imports remain anywhere in codebase. pnpm build passes.
  </done>
</task>
...
6. No deprecated files remain (SettingsModal.tsx, useProjectAIConfig.ts, projectAIConfig.ts)
7. No imports of deprecated modules in any file: `grep -rn "useProjectAIConfig\|projectAIConfig\|AVAILABLE_MODELS\|DEFAULT_MODELS" src/` returns zero
8. `getProvider` import in ProjectWorkspace.tsx resolves correctly
</verification>
...
<output>
After completion, create `.planning/phases/23-settings-ui-split-provider-config/23-03-SUMMARY.md`
</output>
